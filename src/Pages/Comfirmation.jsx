...
import fail from "../assets/noIcon.png";
import success from "../assets/validation.png";
...
function Comfirmation() {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      try {
        const url = `http://localhost:5000/api/confirmation/${param.token}`;
    ...
    <>
      {validUrl ? (
        <div className="text-center flex flex-col justify-center items-center mx-auto text-2xl gap-4 w-full h-full mt-32 ">
          ...
          <Link
            to={"/authentification"}
            className="bg-[#09c231] rounded-xl h-10 text-center px-4 py-1"
          >
            Se Connecter
          </Link>
        </div>
